...
st.write("""# Volume Services Monthly % Var.""")
...
if response.status_code == 200:
    ...
    # Calcular a variação percentual mensal (MoM)
    VolumeServices_MoM = VolumeServices['valor'].pct_change() * 100
    ...
    fig = px.bar(
        ...
        x=mom_data.index,
        y='MoM Change (%)',
        title='Volume Services MoM (%)'
    )

    # Adicionar valores percentuais como texto no gráfico
    fig.update_traces(texttemplate='%{y:.2f}%', textposition='outside')

    # Configurar o intervalo do eixo X
    fig.update_xaxes(range=['2022-01-01', str(mom_data.index.max().date())])
